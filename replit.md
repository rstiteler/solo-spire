...
14. **Class Resources** — Per-class consumable resource tracking for all 12 PHB classes: Rage (Barbarian), Bardic Inspiration (Bard), Channel Divinity/Divine Intervention (Cleric), Wild Shape (Druid), Second Wind/Action Surge/Indomitable/Superiority Dice (Fighter), Ki Points (Monk), Lay on Hands/Divine Sense (Paladin), Sorcery Points/Tides of Chaos (Sorcerer), Arcane Recovery (Wizard); dot tracker (≤10 max) or number +/− controls (>10 max); Short Rest / Long Rest buttons restore appropriate resources; auto-initializes for existing characters; level-up recalculates maxes; AI DM sees resource state and emits STATE_UPDATE to consume them
